...
@export var attack_speed_min := 300.0
@export var attack_speed_max := 400.0
...
func _on_body_entered(body):
	...
	# Just spawn a single projectile when an enemy enters range.
	# Defer spawning so we don't change the physics/area state while queries are being processed.
...
func spawn_attack_for(enemy_body):
	...
	var target_position = enemy_body.global_position + enemy_dir * 5.0
